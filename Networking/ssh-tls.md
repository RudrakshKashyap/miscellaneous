...
_(Replace `$USER` with your username)_
...
- For dynamic port forwarding see [SOCKS proxy file](SOCKSvsVPN.md)
...
_Note: Connection terminates if version mismatch occurs_
...
- Signature:
    [
      exchange hash(of exchange data that containes shared secret) signed with server's private key,
      Proves the server owns the public host key,
    ]
```
...
_All subsequent traffic encrypted with session keys_
